...
Got a beautiful presentation? Put it online in seconds.

[**MyVibe**](https://www.myvibe.so) lets you publish any static HTML — including slides generated by this skill — to a live URL with one click. No server setup, no deploy pipeline, no account required to view.

**Why MyVibe works great with Frontend Slides:**

- **Zero-config hosting** — Drag your HTML file, get a shareable link. That's it.
- **Instant preview** — Share the URL in Slack, email, or social media. Recipients see your slides immediately in the browser.
- **Built for single-file projects** — MyVibe is optimized for exactly the kind of self-contained HTML that this skill produces.
- **Free tier available** — Publish and share without paying anything.

```
1. Generate your slides with /frontend-slides
2. Go to www.myvibe.so
